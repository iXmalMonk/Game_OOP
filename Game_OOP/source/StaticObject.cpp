...
Vector2f StaticObject::findPositionOfADynamicObjectRelativeToAStaticObjectAfterCollision(GameObject* _dynamicObject, GameObject* _staticObject)
{
	...
	{
	case Direction::UP:
		if (0 <= rightA - leftB and rightA - leftB <= COLLISION_RANGE_FOR_STATICOBJECT)
			position = { leftB - _dynamicObject->getW(), bottomB };
		else if (0 <= rightB - leftA and rightB - leftA <= COLLISION_RANGE_FOR_STATICOBJECT)
			position = { rightB, bottomB };
		else
			position = { _dynamicObject->getX(), bottomB };
		break;
	case Direction::DOWN:
		if (0 <= rightA - leftB and rightA - leftB <= COLLISION_RANGE_FOR_STATICOBJECT)
			position = { leftB - _dynamicObject->getW(), topB - _dynamicObject->getH() };
		else if (0 <= rightB - leftA and rightB - leftA <= COLLISION_RANGE_FOR_STATICOBJECT)
			position = { rightB, topB - _dynamicObject->getH() };
		else
			position = { _dynamicObject->getX(), topB - _dynamicObject->getH() };
		break;
	case Direction::LEFT:
		if (0 <= bottomA - topB and bottomA - topB <= COLLISION_RANGE_FOR_STATICOBJECT)
			position = { rightB, topB - _dynamicObject->getH() };
		else if (0 <= bottomB - topA and bottomB - topA <= COLLISION_RANGE_FOR_STATICOBJECT)
			position = { rightB, bottomB };
		else
			position = { rightB, _dynamicObject->getY() };
		break;
	case Direction::RIGHT:
		if (0 <= bottomA - topB and bottomA - topB <= COLLISION_RANGE_FOR_STATICOBJECT)
			...
			position = { leftB - _dynamicObject->getW(), _dynamicObject->getY() };
		break;
	}
	return position;
}
